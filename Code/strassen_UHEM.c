...
void strassen_multiply_internal(double **A, double **B, double **C, int n) {
    ...
    // Perform Strassen's algorithm tasks
    #pragma omp task shared(M1)
    {
        double **T1_local = allocate_matrix(k);
        double **T2_local = allocate_matrix(k);

        matrix_add(A11, A22, T1_local, k);
        matrix_add(B11, B22, T2_local, k);
    ...
    #pragma omp task shared(M6)
    {
        ...
        matrix_sub(A21, A11, T1_local, k);
        matrix_add(B11, B12, T2_local, k);
    ...
    #pragma omp task shared(M7)
    {
        ...
        matrix_sub(A12, A22, T1_local, k);
        matrix_add(B21, B22, T2_local, k);
    ...
    // Combine sub-results
    matrix_add(M1, M4, T1, k);
    matrix_sub(T1, M5, T2, k);
    matrix_add(T2, M7, C11, k);
    
    matrix_add(M3, M5, C12, k);
    matrix_add(M2, M4, C21, k);
    
    matrix_sub(M1, M2, T1, k);
    matrix_add(T1, M3, T2, k);
    matrix_add(T2, M6, C22, k);
