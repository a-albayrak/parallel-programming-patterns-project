...
#define THRESHOLD 64          //Threshold for naive multiply
 ...
 *-------------------------------------------------------------*/
// Standard O(n^3) multiplication: C = A * B
static void naive_multiply(double **A, double **B, double **C, int n) {
    for (int i = 0; i < n; i++){
        for (int j = 0; j < n; j++){
            double sum = 0.0;
            for (int k = 0; k < n; k++){
 ...
 *-------------------------------------------------------------*/
// A helper that can be called both from parallel and sequential code
...
/*-------------------------------------------------------------
 * Test Function (checks correctness vs. naive multiply)
 *-------------------------------------------------------------*/
void test_strassen_omp(int n) {
    double **A      = allocate_matrix(n);
    double **B      = allocate_matrix(n);
    double **C_omp  = allocate_matrix(n);
    ...
    strassen_multiply_omp(A, B, C_omp, n);
    double end = omp_get_wtime();
    double elapsed_strassen = end - start;
    ...
    double elapsed_naive = end - start;
    printf("Time for Naive multiply (n=%d): %.6f s\n", n, elapsed_naive);
    
    // Compare C_omp vs. C_naive
    int correct = 1;
    for (int i = 0; i < n && correct; i++){
        for (int j = 0; j < n && correct; j++){
            if (fabs(C_omp[i][j] - C_naive[i][j]) > 1e-9) {
                correct = 0;
            }
        }
    }
    ...
    if (correct) {
        printf("Result is CORRECT (Parallel Strassen matches Naive).\n");
    } else {
